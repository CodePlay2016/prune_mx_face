...
class Conv2D(mx.gluon.HybridBlock):
    ...
    def __init__(self, channels, kernel_size, strides=(1, 1), padding=(1, 1),
...
class mConv2D(mx.gluon.HybridBlock):
    conv_output = {}
    capture_layer_name = []
    ...
    def __init__(self, channels, kernel_size, strides=(1, 1), padding=(0, 0),
                 ...
                 activation=None, use_bias=True, weight_initializer=None,
                 bias_initializer='zeros', in_channels=0, **kwargs):
        super(mConv2D, self).__init__(**kwargs)
        self.conv = nn.Conv2D(channels, kernel_size, strides=strides, padding=padding,
                              dilation=dilation, groups=groups, layout=layout,
                              activation=activation, use_bias=use_bias, weight_initializer=weight_initializer,
                              bias_initializer=bias_initializer, in_channels=in_channels)
    ...
    def hybrid_forward(self, F, x):
        ...
        name = self._prefix[:-1]
        if name in mConv2D.capture_layer_name:
            mConv2D.conv_output[name] = self.conv(x)
            mConv2D.conv_output[name].attach_grad()
            return mConv2D.conv_output[name]
        else:
            out = self.conv(x)
            return out
...
def _get_grad(net, image, label, criterion=None, conv_layer_name=None, image_grad=False):
    ...
    # one_hot_target = mx.nd.one_hot(mx.nd.array([class_id]), out.shape[1])
    # one_hot_target = mx.nd.tile(one_hot_target,reps=(8,1))
    ...
    if image_grad:
        ...
        return conv_out, conv_out.grad

def _get_grads(net, image, label, criterion=None, conv_layer_name=None, image_grad=False):

    if image_grad:
        image.attach_grad()
        mConv2D.capture_layer_name = None
        Activation.set_guided_backprop(True)
    else:
        # Tell convviz.Conv2D which layer's output and gradient needs to be recorded
        mConv2D.capture_layer_name = conv_layer_name
        Activation.set_guided_backprop(False)
    ...
    if criterion is None:
        out.backward(label,retain_graph=True,train_mode=False)
    else:
        out.backward(retain_graph=True,train_mode=False)
    ...
    if image_grad:
        ...
        # Return the recorded convolution output and gradient
        conv_out =  mConv2D.conv_output
        return conv_out, conv_out.grad


def get_conv_out_grad(net, image, class_id, conv_layer_name=None, criterion=None):
    ...
    conv_layer_name: str
        Name of the convolutional layer whose output and output's gradients need to be acptured."""
    return _get_grad(net, image, class_id, criterion, conv_layer_name, image_grad=False)

def get_conv_out_grads(net, image, class_id, conv_layer_name=None, criterion=None):
    return _get_grads(net, image, class_id, criterion, conv_layer_name, image_grad=False)
